    ...
    async function loadModels(){
      try{
        ...
        if(data.models?.length){
          ...
          setModel(data.models.includes(data.defaultModel)?data.defaultModel:data.models[0]);
        }
      }catch{
        // Keep the model selector empty if loading fails.
      }
    }
  ...
  return <main className="shell">
    <aside className="sidebar">
      <div className="brand">
        ...
        <div><b>Experiential AI</b><span>Chat</span></div>
      </div>
      <button className="newChat" onClick={newChat}>＋ New chat</button>
      <div className="sideInfo">
        <p>Powered by Experiential Labs</p>
        <small>Your API key stays on the server and is never sent to the browser.</small>
      </div>
    </aside>
    ...
    <section className="chat">
      <header className="topbar">
        <div>
          <h1>AI Assistant</h1>
          <p>OpenAI-compatible Experiential Labs gateway</p>
        </div>
